...
PR #224 merged as `df2de03dbc5211c281be5034fde8be04c8718fa7`. Its main Quality and CodeQL workflows passed, and a scan-only release-security run on that exact commit passed all three jobs; see [AWS/hosted attempt](aws-hosted-attempt-2026-09-26.md) for the run, artifacts and bundle digest. The Dependabot alert is resolved. Three production CodeQL alerts resolved through the merge; [alert #3](https://github.com/Collaboration95/rewind-app/security/code-scanning/3) persisted on main and was then dismissed as a documented false positive after reviewing the blob-only guard and regression test. Fresh API queries return zero open CodeQL and zero open Dependabot alerts. Thus earlier statements below describing #224 as awaiting review are historical and no longer current.
