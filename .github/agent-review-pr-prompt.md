...
You may inspect files, run safe local checks, capture relevant screenshots, and
publish one top-level review comment on the PR. Do not edit files, create
commits, change Issue or Project status, approve the PR, merge it, or close or
delete anything.
...
1. Resolve the PR URL and record the PR number, head branch, base branch,
   linked Issue(s), changed files, and current CI/check status.
2. Read repository instructions (`AGENTS.md` or equivalent), linked Issue
...
3. Inspect the complete diff, including tests, documentation, configuration,
   ...
   scope.
4. Run the relevant local checks available in the repository. Prefer commands
   documented by the project. At minimum, run applicable formatting, lint,
   type-check, unit-test, build, or startup checks. Do not claim a check passed
   unless it actually ran and passed.
5. If compilation, type-checking, bundling, or another required check fails,
   continue reviewing the diff and publish the review anyway. Record the exact
...
7. Evaluate the change against these review gates:
   ...
   - **Tests:** tests cover changed behaviour and meaningful negative paths;
   ...
   - **Maintainability:** the design is understandable, appropriately scoped,
     ...
     hidden coupling.
   - **Evidence:** the PR explains how acceptance criteria were verified and
     links screenshots or other evidence when relevant and actually available.
   - **Traceability:** grouped work explains why the Issues belong together,
...
Do not inflate severity to make a preference sound mandatory. Every finding must
include the file and line when available, the concrete problem, why it matters,
and a practical fix direction.
...
- Screenshots/manual evidence: links or `not run` when not applicable
