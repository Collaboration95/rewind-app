...
validate_recreate_plan() {
  ...
  ' "$json_path" >/dev/null; then
    lifecycle_guard_reject 'the recreation plan contains a delete, update, or replacement; refusing to apply it.'
    return 1
  fi
}

plan_file="$(mktemp "${TMPDIR:-/tmp}/rewind-wake.XXXXXX.tfplan")"
plan_json="$(mktemp "${TMPDIR:-/tmp}/rewind-wake.XXXXXX.json")"
...
if [[ "$APPLY" == 0 ]]; then
  printf 'Dry-run passed: the plan is eligible to create only the disposable Demo compute.\n'
